## This script generates panels a-c for Figure 2
## Intercomparison of Turbinaria and water column N
...
## Turbinaria %N significantly varied by habitat type (p < 0.001), with the highest %N at fringing reef sites and in bays and their passes, and lower %N at mid-lagoon and back reef sites (Fig. 2a). 
anova(turb_diffs_lmer)
...
## Water column N also varied by habitat (p < 0.001), showing a similar pattern of variation, with highest nitrite + nitrate in bays, followed by fringing reef, reef pass, and finally mid lagoon and back reef sites (Fig. 2b).  
anova(H2O_diffs_lmer)
...
## In general, Turbinaria %N was positively associated with water column N (Fig. 2c; R2 = 0.14, p < 0.001).
longTermQuestion = compiled_N %>%
  ## Use only years in which both water column and Turbinaria data were collected
  filter(Year %in% c(2021:2023)) %>%
  ## Identify mean N by site
  group_by(Site, Habitat) %>%
...
turb_water_compare = plot_grid(Fig2ab,
                               ...
                               labels = NULL)
turb_water_compare
ggsave("plots/turb_water_compare.jpg",turb_water_compare,
